...
while True:
    ...
    black_null_img = np.zeros((height, width, 3),np.uint8)
    # 背景白にする場合。
    ...
    # 4つの画像を１つのウィンドウに組込む
    #merged_image_group_1 = cv2.hconcat((mask_rcnn_image, objection_detected_image))
    #merged_image_group_2 = cv2.hconcat((black_bachground_img, black_null_img))
    ...
    # 3つの画像を１つのウィンドウに組込む
    upper_sub_window_image = cv2.hconcat((mask_rcnn_image, objection_detected_image))
    lower_sub_window_image = black_bachground_img
    height = upper_sub_window_image.shape[0]
    width = upper_sub_window_image.shape[1]
    
    # 上段の画像と下段の画像の縦横サイズを揃える
    resized_upper_sub_window_image = cv2.resize(upper_sub_window_image, (int(1.7*width), int(1.7*height)))
    resized_lower_sub_window_image = cv2.resize(lower_sub_window_image, (int(1.7*width), int(1.7*height)))
    final_output_image = cv2.vconcat((resized_upper_sub_window_image, resized_lower_sub_window_image))
